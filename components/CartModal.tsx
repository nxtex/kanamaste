...
export default function CartModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { items, totalQty, updateQty, removeItem } = useCart()
            ...
            <div style={{ padding: 'var(--space-5) var(--space-5) var(--space-4)', borderBottom: '1.5px solid var(--color-border)', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-stamp)', fontSize: 9, background: 'var(--color-primary)', color: 'var(--color-text-inverse)', borderRadius: 'var(--radius-full)', padding: '2px 8px', letterSpacing: '0.08em' }}>{totalQty}</span>
                </div>
                ...
                {!confirmed ? (
                  items.length === 0 ? (
                    ...
                    >
                      ...
                      <button onClick={() => handleRemove(item.id)} aria-label="Supprimer" style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 'var(--radius-sm)', color: 'var(--color-text-muted)', border: '1.5px solid var(--color-border)', cursor: 'pointer' }}><Trash2 size={14} /></button>
                    </motion.div>
                  ))
                ) : (
                  <motion.div
